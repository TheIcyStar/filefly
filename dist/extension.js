...
function getConnectionDashboardHtml(title = "New FileFly Instance") {
...
var ConnectionDashboardPanel = class _ConnectionDashboardPanel {
  static viewType = "filefly.connectionDashboard";
  _panel;
  _disposables = [];
  constructor(panel) {
    this._panel = panel;
  ...
  static createOrShow(extensionUri) {
    ...
    const panel = vscode.window.createWebviewPanel(
      _ConnectionDashboardPanel.viewType,
    ...
    new _ConnectionDashboardPanel(panel);
  }
  _handleMessage(msg) {
    switch (msg.command) {
      case "submit":
        this._saveConnection(msg.payload);
        break;
      case "testConnection":
        this._testConnection(msg.payload);
        break;
      case "cancel":
        this.dispose();
        break;
  ...
  _saveConnection(config) {
    const existing = vscode.workspace.getConfiguration("filefly").get("connections") ?? [];
    ...
    vscode.workspace.getConfiguration("filefly").update("connections", existing, vscode.ConfigurationTarget.Global).then(() => {
...
function iconForStatus(status) {
  switch (status) {
    case "connected":
      return new vscode.ThemeIcon("database", new vscode.ThemeColor("testing.iconPassed"));
    case "disconnected":
      return new vscode.ThemeIcon("circle-outline", new vscode.ThemeColor("disabledForeground"));
  }
}
function buildTooltip(config, status) {
...
var ConnectionItem = class extends vscode.TreeItem {
...
var ConnectionTreeProvider = class {
  ...
  setStatus(connectionName, status) {
  ...
  getChildren(element) {
    ...
    const configs = vscode.workspace.getConfiguration("filefly").get("connections") ?? [];
    return configs.map(
      (cfg) => new ConnectionItem(cfg, this.statusMap.get(cfg.connectionName) ?? "disconnected")
    );
  }
};
var currentEditPanel;
var EditConnectionPanel = class {
  static createOrShow(extensionUri, config, context) {
    ...
    currentEditPanel.webview.onDidReceiveMessage(
      (msg) => handleEditMessage(msg, config.connectionName),
      void 0,
      context.subscriptions
    );
    currentEditPanel.onDidDispose(() => {
      currentEditPanel = void 0;
    }, null, context.subscriptions);
  }
};
function handleEditMessage(msg, originalName) {
  switch (msg.command) {
    case "submit": {
      const config = msg.payload;
      const settings = vscode.workspace.getConfiguration("filefly");
      const existing = settings.get("connections") ?? [];
      settings.update(
        "connections",
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      ...
      (item) => EditConnectionPanel.createOrShow(context.extensionUri, item.config, context)
    ),
    vscode.commands.registerCommand(
